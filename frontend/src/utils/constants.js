export const TITLES = ['Admin Assistant', 'Full Stack Developer', 'IT Infrastructure Engineer'];

export const SOCIAL_URLS = {
  github: 'https://github.com/yogeshwaranjs131-ui',
  linkedin: 'https://www.linkedin.com/in/yogeshwaran-udayakumar-25b94b170',
};

export const PORTFOLIO_URLS = {
  profilePhoto: '/my-photo.jpg',
  resume: 'https://drive.google.com/file/d/1v4XasrgcCUTQ7CSRkz7Nk8GNmiNeFndu/view',
  nsdcCertificate: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzhHCKU8uyjyCfIiyeYtJRLMN8QbPlN7kFqBhvJE2fCw&s=10',
};

export const CERTIFICATIONS = {
  guvi: {
    name: 'GUVI Certified',
    logo: 'https://cdn.brandfetch.io/guvi.io/logo',
    link: 'https://drive.google.com/file/d/1SC6Qx0CM__ctupSJRTndkbUAd1ODScNw/view?usp=drivesdk',
    preview: 'https://drive.google.com/uc?export=view&id=1SC6Qx0CM__ctupSJRTndkbUAd1ODScNw',
  },
  nsdc: {
    name: 'NSDC Certified',
    logo: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzhHCKU8uyjyCfIiyeYtJRLMN8QbPlN7kFqBhvJE2fCw&s=10',
    link: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzhHCKU8uyjyCfIiyeYtJRLMN8QbPlN7kFqBhvJE2fCw&s=10',
    preview: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzhHCKU8uyjyCfIiyeYtJRLMN8QbPlN7kFqBhvJE2fCw&s=10',
  },
};
