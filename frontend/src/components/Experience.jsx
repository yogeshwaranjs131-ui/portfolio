              ...
              <div className="flex flex-col md:flex-row justify-between mb-4 gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-slate-800/50 flex items-center justify-center group-hover:bg-blue-600/20 group-hover:shadow-[0_0_20px_rgba(59,130,246,0.3)] group-hover:border-blue-500/50 transition-all overflow-hidden border border-slate-800 shrink-0">
                    {exp.logo ? (
                      <img src={exp.logo} alt={exp.company} className="w-full h-full object-cover p-1 transition-transform duration-300 group-hover:scale-110" />
                    ) : (
                      <Briefcase className="w-6 h-6 text-blue-500 group-hover:text-white" />
                    )}
                  </div>
                  <div>
                    <h3 className="text-lg md:text-xl font-bold text-white leading-tight">{exp.role}</h3>
                    <p className="text-blue-400 font-bold text-sm md:text-base">{exp.company}</p>
                  </div>
                </div>
                <span className="px-4 py-1 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full text-[10px] font-bold h-fit w-fit">{exp.period}</span>
              </div>
              <p className="text-slate-400 leading-relaxed text-sm md:text-base md:ml-20">{exp.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
